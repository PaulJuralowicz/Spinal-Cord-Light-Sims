...
myname      = 'internalHor';% name for files: myname_T.bin, myname_H.mci  
...
waist       = 0.05;  	% 1/e radius of beam at focus

% only used if launchflag == 1 (manually set launch trajectory):
% I want this to be like 45 degrees
...
T = permute(T, [1,3,2]); %for horizontal
%%
if SAVEON
    tic
...
% 
%%% draw launch
% bug, doesn't care about xs
% I also don't think it caes about the launch tragectory
% I've modified it to care
% for now uniform only.
...
switch mcflag
    case 0 % uniform
        for i=0:N
            plot([((-radius + 2*radius*i/N)+xs) ((-radius + 2*radius*i/N)+xs) + deltaZ*zToX],[zs max(z)],'r-')
        end

    case 1 % Gaussian
        for i=0:N
            plot([((-radius + 2*radius*i/N)+xs) xfocus],[zs zfocus],'r-')
        end
